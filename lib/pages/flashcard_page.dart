      ...
      body: uid == null ? const Center(child: Text('Sign in')) : Padding(
        ...
        child: Column(children: [
          ...
          ElevatedButton(onPressed: () {
            ...
            qCtrl.clear(); aCtrl.clear();
          }, child: const Text('Add')),
          const SizedBox(height: 12),
          const Text('Cards', style: AppTextStyles.heading2),
          Expanded(child: StreamBuilder<List<Flashcard>>(
            stream: vm.streamCards(uid),
            builder: (context, snap) {
              if (!snap.hasData) return const Center(child: CircularProgressIndicator());
              final cards = snap.data!;
              if (cards.isEmpty) return const Center(child: Text('No cards'));
              return ListView.builder(itemCount: cards.length, itemBuilder: (ctx,i) {
                final c = cards[i];
                return ListTile(title: Text(c.question), subtitle: Text(c.answer), trailing: IconButton(icon: const Icon(Icons.delete), onPressed: () => vm.deleteCard(c.id)));
              });
            },
          )),
        ]),
